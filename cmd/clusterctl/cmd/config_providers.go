...
func runGetRepository(providerName, targetNamespace, watchingNamespace, output string) error {
	...
	components, err := c.GetProviderConfig(providerName, targetNamespace, watchingNamespace)
	if err != nil {
		return err
	}

	if output == "yaml" {
		return repositoryYAMLOutput(components)
	}
	return repositoryDefaultOutput(components)
}

func repositoryYAMLOutput(c client.Components) error {
	yaml, err := c.Yaml()
	if err != nil {
		return err
	}

	_, err = fmt.Fprintln(os.Stdout, string(yaml))
	return err
}

func repositoryDefaultOutput(c client.Components) error {
	fmt.Printf("Name:               %s\n", c.Name())
	fmt.Printf("Type:               %s\n", c.Type())
	fmt.Printf("URL:                %s\n", c.URL())
