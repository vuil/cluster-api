...
var configClusterClusterCmd = &cobra.Command{
	...
	Example: Examples(`
		# Generates a yaml file for for creating a cluster API workload clusters using
		...
		# Generates a yaml file for for creating a cluster API workload clusters in the "foo" namespace.
		clusterctl config cluster my-cluster --target-namespace=foo
		
		# Generates a yaml file for for creating a cluster API workload clusters with
		# nodes using v1.16.0 Kubernetes version
...
func init() {
	...
	//TODO: Default for kubernetes version??
...
func runGenerateCluster(name string) error {
	...
	fo := client.TemplateOptions{
		ClusterName:       name,
		Namespace:         cc.targetNamespace,
		KubernetesVersion: cc.kubernetesVersion,
		ControlplaneCount: cc.controlplaneCount,
		WorkerCount:       cc.workerCount,
	}

	template, err := c.GetClusterTemplate(cc.kubeconfig, cc.infrastructureProvider, cc.flavor, cc.bootstrapProvider, fo)
	if err != nil {
		return err
	}

	os.Stdout.WriteString(string(template.Yaml()))
