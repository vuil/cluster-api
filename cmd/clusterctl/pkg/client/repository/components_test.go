...
func Test_kustomizeBuild(t *testing.T) {
	type args struct {
	...
	tests := []struct {
		...
		{
			name: "nested folder (e.g. kubebuilder generate config)",
			args: args{
				files: map[string][]byte{
					"default/kustomization.yaml": []byte("apiVersion: kustomize.config.k8s.io/v1beta1\n" +
						"kind: Kustomization\n" +
						"resources:\n" +
						"- ../crd\n"),
					"crd/kustomization.yaml": []byte("apiVersion: kustomize.config.k8s.io/v1beta1\n" +
						"kind: Kustomization\n" +
						"resources:\n" +
						"- a.yaml\n"),
					"crd/a.yaml": podYaml,
				},
				kustomizeDir: "default",
			},
			want:    podYaml,
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := kustomizeBuild(tt.args.files, tt.args.kustomizeDir)
			if (err != nil) != tt.wantErr {
				t.Errorf("kustomizeBuild() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("kustomizeBuild() got = %s, want %s", string(got), string(tt.want))
			}
		})
	}
}
