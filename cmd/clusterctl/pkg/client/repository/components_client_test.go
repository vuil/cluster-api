...
func Test_componentsClient_Get(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				...
				repository: test.NewFakeRepository().
					...
					WithPaths("root", "", "").
					WithFile("v1.0.0", "components.yaml", util.JoinYaml(namespaceYaml, podYaml, configMapYaml)).
					WithFile("v1.0.0", "kustomization.yaml", []byte("apiVersion: kustomize.config.k8s.io/v1beta1\n"+
						"kind: Kustomization\n"+
						"resources:\n"+
						"- components.yaml\n")),
				configVariablesClient: test.NewFakeVariableClient().WithVar(variableName, variableValue),
		...
		{
			name: "Pass with nested folder",
			fields: fields{
				provider: p1,
				repository: test.NewFakeRepository().
					WithDefaultVersion("v1.0.0").
					WithPaths("root", "", "default").
					WithFile("v1.0.0", "crd/components.yaml", util.JoinYaml(namespaceYaml, podYaml, configMapYaml)).
					WithFile("v1.0.0", "crd/kustomization.yaml", []byte("apiVersion: kustomize.config.k8s.io/v1beta1\n"+
						"kind: Kustomization\n"+
						"resources:\n"+
						"- components.yaml\n")).
					WithFile("v1.0.0", "default/kustomization.yaml", []byte("apiVersion: kustomize.config.k8s.io/v1beta1\n"+
						"kind: Kustomization\n"+
						"resources:\n"+
						"- ../crd\n")),
				configVariablesClient: test.NewFakeVariableClient().WithVar(variableName, variableValue),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// TODO: check targetNamespace applied to components

			// TODO: check variable replaced in components

			//TODO: check targetNamespace in objects

			//TODO: check labels in objects
		})
	}
}
