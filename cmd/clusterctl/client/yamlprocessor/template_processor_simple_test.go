...
func TestSimpleProcessor(t *testing.T) {
	...
	t.Run("returns expected artifact name", func(t *testing.T) {
		g := NewWithT(t)
		p := NewSimpleProcessor()
		g.Expect(p.ArtifactName("some-version", "some-flavor")).To(Equal("cluster-template-some-flavor.yaml"))
		g.Expect(p.ArtifactName("", "")).To(Equal("cluster-template.yaml"))
	})

	t.Run("returns the variables from the yaml", func(t *testing.T) {
		type args struct {
			data string
		}
		tests := []struct {
	...
	t.Run("replaces the variables with the values", func(t *testing.T) {
		type args struct {
			yaml                  []byte
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				got, err := p.Process(tt.args.yaml, tt.args.configVariablesClient)
				if tt.wantErr {
