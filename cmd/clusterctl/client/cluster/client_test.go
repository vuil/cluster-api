...
func Test_newClusterClient_YamlProcessor(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts: []Option{InjectYamlProcessor(&fakeProcessor{})},
			assert: func(g *WithT, p yaml.Processor) {
				_, ok := (p).(*yaml.SimpleProcessor)
				g.Expect(ok).To(BeFalse())
				_, ok = (p).(*fakeProcessor)
				g.Expect(ok).To(BeTrue())
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			opts := append(
				tt.opts,
				InjectProxy(test.NewFakeProxy()),
				InjectPollImmediateWaiter(fakePollImmediateWaiter),
			)

			client := newClusterClient(Kubeconfig{}, &fakeConfigClient{}, opts...)
